...
export const searchSpsApiOauthV2 = createAsyncThunk(
  ...
  async (searchParam?: ISearchSpsApiOauthV2) => {
    const response = await apiOauthV2Service
      .searchSpsApiOauthV2(searchParam)
      .then((res) => {
        return res.body;
      });
...
export const getSpsApiOauthV2ById = createAsyncThunk(
  'getSpsApiOauthV2ById',
  async (id: number) => {
    const response = await apiOauthV2Service
      .getSpsApiOauthV2ById(id)
      .then((res) => {
        return res.body;
      });
    return response.data;
...
export const deleteSpsApiOauthV2 = createAsyncThunk(
  'deleteSpsApiOauthV2',
  async (id: number) => {
    const response = await apiOauthV2Service
      .deleteSpsApiOauthV2(id)
      .then((res) => {
        return res.body;
      });
    return response;
