...
export const globalTableColumnSelectionSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getGlobalTableColumns.fulfilled.type]: (
      ...
      if (action.payload) {
        state.getGlobalTableColumns.data.id = action.payload.id;
        state.getGlobalTableColumns.data.columns = JSON.parse(action.payload.columns as any);
        state.getGlobalTableColumns.data.table_name = action.payload.table_name;
      }
