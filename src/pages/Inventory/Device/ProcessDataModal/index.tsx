...
import { Common, TableNames, validateMessages } from '../../../../common/constants/common';
...
const ProcessDataModal: React.FC<IProcessDataModalProps> = (props) => {
  ...
  const initialValues = {
    company_id: null,
    bu_id: null,
    date_added: getSimpleDate().format(Common.DATEFORMAT),
  ...
  const onFinish = (values: any) => {
    ...
    dispatch(processDataDevice(values));
  };
  ...
  useEffect(() => {
    if (device.processData.messages.length > 0) {
      if (device.processData.hasErrors) {
        toast.error(device.processData.messages.join(' '));
      } else {
        toast.warning(device.processData.messages.join(' '));
        handleModalClose();
      }
      dispatch(clearDeviceMessages());
    }
  }, [device.processData.messages]);
        ...
        >
          <Row gutter={[30, 15]} className="form-label-hide">
            ...
            {/* <Col xs={24} sm={12} md={8}>
              <div className="form-group m-0">
                <label className="label">Selected Date </label>
                <Form.Item name="date_added" label="Selected Date " className="m-0" rules={[{ required: true }]}>
          ...
          <div className="btns-block modal-footer pt-lg">
            <Button
              ...
              type="primary"
              htmlType="submit"
              loading={device.processData.loading}
            >
              Process
            </Button>
