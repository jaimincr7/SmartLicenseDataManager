...
import { IAddO365ProductListProps } from './addO365ProductList.model';

const { Option } = Select;

const AddO365ProductListModal: React.FC<IAddO365ProductListProps> = (props) => {
  ...
  let initialValues: IO365ProductList = {
    tenant_id: null,
    company_id: null,
    bu_id: null,
    product_title: '',
  ...
  const onFinish = (values: any) => {
    const inputValues: IO365ProductList = {
      ...values,
      id: id ? +id : null,
    };
    if (!isMultiple) {
  ...
  const fillValuesOnEdit = async (data: IO365ProductList) => {
    if (data.tenant_id) {
  ...
  useEffect(() => {
    if (o365ProductList.save.messages.length > 0) {
      if (o365ProductList.save.hasErrors) {
        toast.error(o365ProductList.save.messages.join(' '));
      } else {
        toast.success(o365ProductList.save.messages.join(' '));
        handleModalClose();
        refreshDataTable();
      }
      dispatch(clearO365ProductListMessages());
    }
  }, [o365ProductList.save.messages]);
  ...
  useEffect(() => {
    if (+id > 0 && o365ProductList.getById.data) {
      const data = o365ProductList.getById.data;
  ...
  useEffect(() => {
    ...
    if (+id > 0) {
      dispatch(getO365ProductListById(+id));
    }
    return () => {
      dispatch(clearO365ProductListGetById());
      dispatch(clearCompanyLookUp());
      dispatch(clearBULookUp());
    };
  }, [dispatch]);
  ...
  return (
    <>
      <Modal
        ...
        footer={false}
      >
        {o365ProductList.getById.loading ? (
          <div className="spin-loader">
            <Spin spinning={o365ProductList.getById.loading} />
          </div>
        ) : (
          <Form
            form={form}
            name="addO365ProductList"
...
export default AddO365ProductListModal;
