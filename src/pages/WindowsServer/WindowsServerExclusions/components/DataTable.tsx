...
const DataTable: React.ForwardRefRenderFunction<unknown, IDataTable> = (props, ref) => {
  ...
  const getSearchData = (page, isExportToExcel: boolean) => {
    ...
    const searchData: ISearchWindowsServerExclusions = {
  ...
  const fetchWindowsServerExclusions = (page = null) => {
    const searchData = getSearchData(page, false);
    dispatch(searchWindowsServerExclusions(searchData));
  };
  useImperativeHandle(ref, () => ({
    refreshData() {
      fetchWindowsServerExclusions();
    },
  }));
  React.useEffect(() => {
    return () => {
  ...
  React.useEffect(() => {
    ...
    fetchWindowsServerExclusions({ ...pagination, current: 1 });
  ...
  const handleTableChange = (paginating, filters, sorter) => {
    ...
    fetchWindowsServerExclusions(paginating);
  };
  ...
  React.useEffect(() => {
    if (windowsServerExclusions.delete.messages.length > 0) {
      if (windowsServerExclusions.delete.hasErrors) {
        toast.error(windowsServerExclusions.delete.messages.join(' '));
      } else {
        toast.success(windowsServerExclusions.delete.messages.join(' '));
        fetchWindowsServerExclusions();
      }
      dispatch(clearWindowsServerExclusionsMessages());
    }
  }, [windowsServerExclusions.delete.messages]);
  // End: Delete action
  ...
  const downloadExcel = () => {
    ...
    return windowsServerExclusionsService.exportExcelFile(searchData).then((res) => {
      if (!res) {
  ...
  const columns = [
    ...
    },
    {
      ...
      children: [
        {
          title: FilterByDropdown('company_id', windowsServerExclusions.search.lookups?.companies),
    ...
    },
    {
      ...
      children: [
        {
          title: FilterByDropdown('enabled', windowsServerExclusions.search.lookups?.booleanLookup),
          dataIndex: 'enabled',
          key: 'enabled',
          ellipsis: true,
      ...
      children: [
        {
          ...
          render: (_, data: IWindowsServerExclusions) => (
            <div className="btns-block">
              <a
    ...
    <>
      ...
      <Form form={form} initialValues={inlineSearch} name="searchTable" onFinish={onFinish}>
        <Table
          ...
          dataSource={windowsServerExclusions.search.data}
          columns={getColumns()}
          loading={windowsServerExclusions.search.loading}
          pagination={{
            ...pagination,
            total: windowsServerExclusions.search.count,
